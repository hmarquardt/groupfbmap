...
let region, ddbClient, docClient, s3Client;
try {
    region = process.env.AWS_REGION || "us-west-2"; // Note: AWS_REGION is usually set by Lambda runtime
    console.log("DEBUG: Initializing DynamoDBClient...");
    ddbClient = new DynamoDBClient({ region });
    console.log("DEBUG: Initializing DynamoDBDocumentClient...");
    docClient = DynamoDBDocumentClient.from(ddbClient);
    console.log("DEBUG: Initializing S3Client...");
    s3Client = new S3Client({ region });
    console.log("DEBUG: AWS Clients initialized successfully.");
} catch (initError) {
    console.error("FATAL: Error initializing AWS Clients:", initError);
    // We cannot proceed if clients fail to initialize
    throw initError; // Re-throw to cause a proper Lambda initialization failure if needed
}
...
exports.handler = async (event) => {
    ...
    if (profile_picture_s3_key) {
        console.log("DEBUG: Entered 'if (profile_picture_s3_key)' block."); // SIMPLIFIED LOG
        // Validate the key format roughly if desired (e.g., check prefix)
        // Add detailed logging before URL construction
        console.log("DEBUG: Checking key format before URL construction."); // SIMPLIFIED LOG
        if (typeof profile_picture_s3_key === 'string' && profile_picture_s3_key.startsWith('avatars/')) {
             ...
             console.log("DEBUG: Successfully constructed profile_picture_url."); // SIMPLIFIED LOG
        } else {
            console.warn("DEBUG: Invalid profile_picture_s3_key format detected."); // SIMPLIFIED LOG
            // Keep profile_picture_url as null if key is invalid
        }
    } else { // Moved this else block to correspond to the outer if (line 76)
        console.log("DEBUG: 'if (profile_picture_s3_key)' evaluated to false."); // SIMPLIFIED LOG
    }
